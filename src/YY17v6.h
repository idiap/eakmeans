#ifndef PLL_ELKANKMEANS_17V6_H
#define PLL_ELKANKMEANS_17V6_H
			...
			if (globallower < upb[i]){
				for (TInt gi = 0; gi < ngroups; ++gi){
					if (glowers[i*ngroups + gi] < upb[i]){
						...
						/* (1) set group nearest, second nearest, index of nearest */
...
template <typename TInt, typename TFloat>
class P17V6 : public kmeans::BaseYYSMN<TInt, TFloat>{
	

	protected:	
		
		std::function<void(TInt)> update_L_glowers_upb_S_H_17v6_ati(){

			return [this](TInt ti){
				TInt x0 = (ti*this->getndata())/this->getnthreads();
				this->pll_principal_X(update_L_glowers_upb_S_H_17v6<TInt, TFloat>, ti, this->get_delta_C(), this->get_delta_G(), this->get_L() + x0, this->get_ngroups(), this->get_groupparts(), this->get_groupsizes(), this->get_group() + x0, this->get_glowers() + x0*this->get_ngroups(), this->get_upb() + x0, this->round);
	...
	public:
		...
		P17V6(Args&&... args): BC(std::forward<Args>(args)...)
		
		{
			this->setalgname("p17v6");
		}
		
		virtual ~P17V6(){}

		virtual void verbose_write_additional(){
			this->get_verbose_file() << "\n\n ..not implemented down to 17v6..\n\n";
		}
		...
		virtual void set_X_tasks(){
			...
			this->X_tasks = {
				this->update_L_glowers_upb_S_H_17v6_ati()
			};
		}
};
